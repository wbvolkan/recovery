...
def stand1(solafis,sagafis, tv, logo,standName): # perskeptif hale getirilip importlanacak fotoğrafların isimlerini parametre olarak bekliyoruz
	...
	if os.path.exists(solafis+".png"):
		...
		coeffs = find_coeffs(
			...
			[(0, height * 0.032), (width, 0), (width, height), (0, height * 0.984)]) 			# fotoğrafın oynanmamış ölçülerini 1. listede , perspektif değerlerinide 2. listede find_coeffs fonksiyonuna gönderiyoruz
		...
		transform = transform.resize((178, 208),Image.ANTIALIAS)  								# perspektif verilen fotoğrafı yeniden boyutlandırıyoruz
	...
	if os.path.exists(sagafis+".png"):
		...
		coeffs = find_coeffs(
			[(0, 0), (width, 0), (width, height), (0, height)],
			[(0, 0), (width, height * 0.032), (width, height * 0.994), (0,height)])


		transform = photo2.transform((width,height),Image.PERSPECTIVE, coeffs,Image.BICUBIC)
		transform = transform.resize((183, 208),Image.ANTIALIAS)
	...
	if os.path.exists(tv+".png"):
		...
		coeffs = find_coeffs(
			[(0, 0), (width, 0), (width, height), (0, height)],
			[(0, height * 0.165), (width, 0), (width, height * 0.930), (0,height)])


		transform = photo3.transform((width,height),Image.PERSPECTIVE, coeffs,Image.BICUBIC)
		transform = transform.resize((115, 68),Image.ANTIALIAS)
	...
	if os.path.exists(logo+".png"):
		...
		coeffs = find_coeffs(
			[(0, 0), (width, 0), (width, height), (0, height)],
			[(0, 0), (width, height * 0.042), (width, height), (0, height * 0.860)])


		transform = photo4.transform((width,height),Image.PERSPECTIVE, coeffs,Image.BICUBIC)
		transform = transform.resize((118, 104),Image.ANTIALIAS)
