...
def stand1(solafis,sagafis, tv, logo,standName): # perskeptif hale getirilip importlanacak fotoğrafların isimlerini parametre olarak bekliyoruz
	...
	if os.path.exists(solafis+".png"):
		...
		coeffs = find_coeffs(
			...
			[(0, height * 0.041), (width, 0), (width, height), (0, height * 0.973)]) 			# fotoğrafın oynanmamış ölçülerini 1. listede , perspektif değerlerinide 2. listede find_coeffs fonksiyonuna gönderiyoruz
		...
		transform = transform.resize((160, 194),Image.ANTIALIAS)  								# perspektif verilen fotoğrafı yeniden boyutlandırıyoruz
	...
	if os.path.exists(sagafis+".png"):
		...
		coeffs = find_coeffs(
			[(0, 0), (width, 0), (width, height), (0, height)],
			[(0, 0), (width, height * 0.034), (width, height * 0.973), (0,height)])


		transform = photo2.transform((width,height),Image.PERSPECTIVE, coeffs,Image.BICUBIC)
		transform = transform.resize((161, 192),Image.ANTIALIAS)
	...
	if os.path.exists(tv+".png"):
		...
		transform = transform.resize((132, 65),Image.ANTIALIAS)
