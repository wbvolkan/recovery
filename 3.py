...
def stand1(solafis,sagafis, tv, logo,standName): # perskeptif hale getirilip importlanacak fotoğrafların isimlerini parametre olarak bekliyoruz
	...
	if os.path.exists(solafis+".png"):
		...
		coeffs = find_coeffs(
			...
			[(0, height - 383), (width, 0), (width, height), (0, height * 0.975)]) 			# fotoğrafın oynanmamış ölçülerini 1. listede , perspektif değerlerinide 2. listede find_coeffs fonksiyonuna gönderiyoruz
		...
		transform = transform.resize((171, 205),Image.ANTIALIAS)  								# perspektif verilen fotoğrafı yeniden boyutlandırıyoruz
	...
	if os.path.exists(sagafis+".png"):
		...
		coeffs = find_coeffs(
			[(0, 0), (width, 0), (width, height), (0, height)],
			[(0, 0), (width, height * 0.054), (width, height * 0.972), (0,height)])


		transform = photo2.transform((width,height),Image.PERSPECTIVE, coeffs,Image.BICUBIC)
		transform = transform.resize((161, 200),Image.ANTIALIAS)
	...
	if os.path.exists(tv+".png"):
		...
		coeffs = find_coeffs(
			[(0, 0), (width, 0), (width, height), (0, height)],
			[(0, 0), (width	, height * 0.019), (width, height), (0,height)])


		transform = photo3.transform((width,height),Image.PERSPECTIVE, coeffs,Image.BICUBIC)
		transform = transform.resize((139, 69),Image.ANTIALIAS)
	...
	if os.path.exists(logo+".png"):
		...
		coeffs = find_coeffs(
			[(0, 0), (width, 0), (width, height), (0, height)],
			[(0, height - 392), (width, 0), (width, height  - 12), (0, height)])


		transform = photo4.transform((width,height),Image.PERSPECTIVE, coeffs,Image.BICUBIC)
		transform = transform.resize((178, 91),Image.ANTIALIAS)
