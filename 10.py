...
def stand1(solafis,sagafis, tv, logo,standName): # perskeptif hale getirilip importlanacak fotoğrafların isimlerini parametre olarak bekliyoruz
	...
	if os.path.exists(solafis+".png"):
		...
		coeffs = find_coeffs(
			...
			[(0, height * 0.070), (width, 0), (width, height), (0, height * 0.985)]) 			# fotoğrafın oynanmamış ölçülerini 1. listede , perspektif değerlerinide 2. listede find_coeffs fonksiyonuna gönderiyoruz
		...
		transform = transform.resize((157, 196),Image.ANTIALIAS)  								# perspektif verilen fotoğrafı yeniden boyutlandırıyoruz
	...
	if os.path.exists(sagafis+".png"):
		...
		coeffs = find_coeffs(
			[(0, 0), (width, 0), (width, height), (0, height)],
			[(0, 0), (width, height * 0.057), (width, height * 0.980), (0,height)])


		transform = photo2.transform((width,height),Image.PERSPECTIVE, coeffs,Image.BICUBIC)
		transform = transform.resize((158, 195),Image.ANTIALIAS)
	...
	if os.path.exists(tv+".png"):
		...
		transform = transform.resize((138, 65),Image.ANTIALIAS)
	...
	if os.path.exists(logo+".png"):
		...
		coeffs = find_coeffs(
			...
			[(0, 0), (width, 0), (width, height), (0, height * 0.985)])


		transform = photo4.transform((width,height),Image.PERSPECTIVE, coeffs,Image.BICUBIC)
		transform = transform.resize((175, 69),Image.ANTIALIAS)
